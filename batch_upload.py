...
def sync_with_remote() -> bool:
    """推送前先同步远程（fetch + pull），避免多进程/多终端并发推送导致的冲突"""
    print("正在同步远程状态 (git fetch)...")
    success, _ = run_command(["git", "fetch", REMOTE_NAME, BRANCH_NAME])
    if not success:
        print("⚠️  无法连接远程仓库，跳过同步，直接尝试推送。")
        return True

    print("正在合并远程更新 (git pull)...")
    success, output = run_command(["git", "pull", "--no-rebase", REMOTE_NAME, BRANCH_NAME])
    if not success:
        print("❌ 自动合并远程更改失败（可能存在冲突），请手动执行 git pull 解决冲突后重新运行脚本。")
        print(output)
        return False
    return True
...
def process_batch(file_list, batch_num, batch_size):
    ...
    if to_rm:
        ...
        for i in range(0, len(to_rm), chunk_size):
            ...
            cmd = ["git", "rm", "--ignore-unmatch"] + chunk
            success, _ = run_command(cmd)
            if not success:
                sys.exit(1)
    ...
    if not success:
        # 推送被拒绝，很可能是同步之后远程又有了新提交（并发推送），重试一次
        print("推送被拒绝，尝试再次同步并重试...")
        if not sync_with_remote():
            sys.exit(1)
        success, err = run_command(cmd)
