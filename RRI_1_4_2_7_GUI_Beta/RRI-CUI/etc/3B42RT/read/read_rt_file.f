...
C
     ...
     +                  i1dum
        ...
        CHARACTER*2880  head
        integer*4       jleft, ibottom, jright, itop
C
...
C
C  15   WRITE (*, *) 'Enter data type (3B40RT, 3B41RT, 3B42RT):'
   15   CONTINUE
        READ (*, 10) type
...
C
        IF  ( type (1:6) .EQ. '3B40RT' )  THEN
            CALL GET_ARRS_FILE ( name,    type,   LN,    LT90, 
     +				 RECLSZ,  head,   p, e,
     +                           npixel,  nambig, nrain, 
     +                           nsource, r4dum,  iret )
            IF  ( iret .LT. 0 )  STOP
...
C
          ELSE IF  ( type (1:6) .EQ. '3B41RT' )  THEN
            CALL GET_ARRS_FILE ( name,   type,  LN,    LT60,  
     +				  RECLSZ, head,  p, e,
     +                            npixel, i1dum, i1dum, 
     +                            i1dum,  r4dum, iret )
            IF  ( iret .LT. 0 )  STOP
...
C
  ...
  110           FORMAT ( 2I5, 2F10.2, I4 )
            ...
            END DO
          ELSE
C           WRITE (*, *) 'i, j, p, e, nsource, p_uncal:'
C           i1 = LN   / 2
C           j1 = LT60 / 2
C           DO  j = j1, j1+1
C           DO  i = i1, i1+1
C           DO  j = 1, LT60
            DO  j = itop, ibottom
             WRITE (*, 120) ( p (i, j), i = jleft, jright )
            END DO
C 120           FORMAT ( 2I5, 2F10.2, I4, F10.2 )
  120           FORMAT ( 1440F10.2 )

C           DO  i = 1, LN
C           DO  j = 1, LT60
C               WRITE (*, 110) i, j, p (i, j), e (i, j), npixel (i, j)
C           ENDDO
C           ENDDO
        END IF
C
        ...
        END
C
        SUBROUTINE GET_ARRS_FILE  ( name, type,  LN, LT,  RECLSZ,
     +				    head, p, e,  np1,     np2,
...
C* R*4.                                                             *
C*                                                                  *
C* GET_ARRS_FILE  ( name, type, LN,  LT,  RECLSZ,  head, p, e,      *
...
cc
	INTEGER*2	ivarin, ivar
	CHARACTER*1	cvarin (2), cvar (2)
	EQUIVALENCE     (cvarin, ivarin)
	EQUIVALENCE     (cvar,   ivar)
cc
...
C       The logical record is the entire set of fields.  lrec is 
C       sized as follows:
...
C       companion field 4 (field of I*1):  LT * LN / 4 words
C       p_uncal           (field of I*2):  LT * LN / 2 words
C
...
C
        ...
        END DO
C
        lrec = LN / 2 + 2 * LT * LN / 2 + LT * LN / 4
        IF  ( type (1:6) .EQ. '3B40RT' )  
     +      lrec = lrec + 3 * LT * LN / 4
        IF  ( type (1:6) .EQ. '3B42RT' )  
     +      lrec = lrec + LT * LN / 2
C
...
cc
        ...
        END DO
C
C       Convert the error from scaled (by 100) I*2.
C
cc
cc      If byte swapping is required, uncomment this section.
cc
...
cc
        ...
        END DO
C
C       Convert p_uncal from scaled (by 100) I*2 for 3B42RT.
C
...
cc
            DO  j = 1, LT
            DO  i = 1, LN
                IF  ( i2u ( i, j ) .EQ. I2MISS )  THEN
                    p_uncal (i, j) = RMISS
                  ELSE
                    p_uncal (i, j) = i2u ( i, j ) * .01
                END IF
            END DO
            END DO
        END IF
